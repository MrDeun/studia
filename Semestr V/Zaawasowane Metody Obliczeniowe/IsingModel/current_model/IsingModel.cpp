...
#define MODEL_TEMPLATE template<size_t width, size_t height>

MODEL_TEMPLATE
double SimulationModel<width, height>::calculateMagnet() {
  int magnet = std::reduce(Model.begin(), Model.end(), 0.0, std::plus{});
  return double(magnet) / double(ModelSize);
}

MODEL_TEMPLATE
int SimulationModel<width, height>::calculateEnergy() {
  int res = 0;
...
MODEL_TEMPLATE
void SimulationModel<width,height>::Simulate(size_t iterations, int bucket){
