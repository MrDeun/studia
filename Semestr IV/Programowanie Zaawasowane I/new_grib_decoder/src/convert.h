...
namespace convert {
    ...
    uint32_t to_long(std::vector<uint8_t>::iterator index);
    uint16_t to_short(std::vector<uint8_t>::iterator index);
    float to_float(std::vector<uint8_t>::iterator index);
    float to_float3(std::vector<uint8_t>::iterator index);
};
